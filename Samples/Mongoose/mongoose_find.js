...
mongoose.connection.once('open', function() {
    ...
    query.exec(function(err, count) {
        console.log("There are " + count + " words that start and end with a vowel");
    });

    query.find().limit(5).sort( {size:-1});
    query.exec(function(err, docs) {
        console.log("Longest 5 words that start and end with a vowel: ");
        for(var i in docs) {
    ...
    query.exec(function(err, docs) {
        console.log("Words with even lengths that are longer than 6 letters: ");
        for(var i in docs) {
